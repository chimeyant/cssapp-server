...
export default class SlidersController {
  public async index({}: HttpContextContract) {
    const sliders = await Slider.query().select('id','title','status').orderBy('id','desc')
  ...
  public async show({params}: HttpContextContract) {
    ...
    const data = {
      ...
      path:slider?.path,
      path_url: Env.get("BASE_URL")+ "/images/sliders/"+ slider?.path,
  ...
  public async publish({}:HttpContextContract){
    ...
    sliders.forEach(async (item)=>{
      const row ={}
      const url = await Drive.getUrl("images/sliders/"+ item.path)
      row['id']= item.id
