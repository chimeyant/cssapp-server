...
export default class UsersController {
  ...
  public async userInfo({auth}: HttpContextContract){
    ...
    const peserta = user?.authent=='peserta'?  await Peserta.findBy("user_uuid", user?.id):null
    const lokasi = user?.authent=='peserta'?  await Lokasi.findBy("uuid", peserta?.lokasiUuid):null
    const data ={}

  if(user?.authent=='peserta'){
    await QRCode.toDataURL(peserta?.noreg)
    .then(url => {
      console.log(url)
      data['qrcode']= url
    })
    ...
    data['lokasi']= lokasi?.name
    data['avatar']= user?.avatar

    data['avatar_path'] = user?.avatar ? Env.get("BASE_URL")+ "/uploads/images/avatars/"+ user?.avatar:   "/images/pencaker.png"
    return data;
  }
